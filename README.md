...
> Every developer who has run a coding agent for more than an hour has watched it get slow,
> expensive, and dumb. The transcript fills up, the tool output piles on, and the agent starts
> re-reading its own stale history instead of doing the job. **Lethe is an agent runtime built
> so that never happens**: the agent keeps a small, explicit working state that it edits itself,
> everything else has an expiry date, and nothing is ever in the prompt that isn't needed for the
> current step. It has been running the same job unattended for hours with a context that never grows.

Built for the Long Horizon Agents Hackathon. The workload that proves it is a real one: a
dealership's used-vehicle pricing agent that watches the **live web market** for hours and keeps
its recommendations current. The runtime is mission-agnostic; the mission is a YAML file.

## The problem, and the three fixes

Long-horizon agents break down over time: observations, actions, and stale context pile up until
the growing history slows them down, costs more, and makes their own state less reliable. Lethe
applies the three fixes directly:

| Fix | Where it lives in Lethe |
|---|---|
| **Explicit mutable state instead of an ever-growing history** | The Mission Ledger: one ~1k-token object, rewritten every cycle, stored in RawTree. No component keeps a message list. Every LLM call is built fresh and refused above `TOKEN_BUDGET`. |
| **Agents that edit their own working context** | The Chief only changes state through tools: `update_plan`, `record_decision`, `add_lesson`, `request_scout`, `set_recommendation`, `mark_stale`. Context assembly ranks and budgets what goes in and logs what was dropped. |
| **A clear split between what must persist and what can be discarded** | Three tiers. *Discard*: raw web pages, gone when the scout finishes. *Expiring*: extracted facts with a TTL (a price lives 2h). *Persistent*: facts confirmed by two independent sources, plus decisions and lessons. A sold listing wipes every fact about it. |

It is a real agent in the plan / act / observe / self-correct sense: it plans which units to
check and how, acts on the web through Nimble, observes through structured Finding Cards, and
when a strategy fails the Auditor writes a lesson and the agent changes strategy on the next
attempt, without a human in the loop.
...
1. *"You've all watched a coding agent get slower and dumber after an hour. That's the history
   growing. Lethe is a runtime where that can't happen."*
